...
export default function Wiki() {
  ...
  const fetchPages = async () => {
    try {
      const res = await fetch(`https://kodo-1jlt.onrender.com/api/wiki/project/${projectId}`, {
  ...
  const fetchHistory = async (pageId) => {
    ...
    try {
      const res = await fetch(`https://kodo-1jlt.onrender.com/api/wiki/${pageId}/history`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      const data = await res.json();
      setHistory(data);
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('https://kodo-1jlt.onrender.com/api/wiki', {
        method: 'POST',
  ...
  const handleSave = async () => {
    try {
      await fetch(`https://kodo-1jlt.onrender.com/api/wiki/${selected.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('token')}` },
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://kodo-1jlt.onrender.com/api/upload/image', {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        body: formData,
      });
