...
export default function Board() {
  ...
  const fetchAttachments = async (taskId) => {
    try {
      const res = await fetch(`https://kodo-1jlt.onrender.com/api/attachments/task/${taskId}`, {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://kodo-1jlt.onrender.com/api/attachments/task/${selectedTask.id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        body: formData,
      });
  ...
  const handleDeleteAttachment = async (id) => {
    try {
      await fetch(`https://kodo-1jlt.onrender.com/api/attachments/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
